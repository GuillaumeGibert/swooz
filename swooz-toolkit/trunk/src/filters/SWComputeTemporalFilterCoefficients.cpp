...
#include "filters\SWComputeTemporalFilterCoefficients.h"

using namespace SWPlugins;
...
void CComputeTemporalFilterCoefficients::findSPlanePolesAndZeros(void)
{
	...
	if(m_ui64FilterMethod==OVP_TypeId_FilterMethod_Butterworth)//poles equally spaced around the unit circle
	{
		...
		{
			l_float64m = pi / (2.0*(double)m_ui32FilterOrder);
		}
		...
		for(uint32 i=0; i<m_ui32NbPoles; i++ )// poles
		{
			...
			l_float64m += pi /(double)m_ui32FilterOrder;
		}
	...
	if(m_ui64FilterMethod==OVP_TypeId_FilterMethod_Chebychev)
	{
		...
		m_float64Phi = pow( m_float64Phi, (double)1.0/m_ui32FilterOrder );  // raise to the 1/n power
...
void CComputeTemporalFilterCoefficients::convertSPlanePolesAndZerosToZPlane(void)
{
	...
	double* l_float64pp = new double[m_ui32ArraySize];
	double* l_float64y = new double[m_ui32ArraySize];
	double* l_float64aa = new double[m_ui32ArraySize];

	double l_float64C=0.0;
	double l_float64a=0.0;
	double l_float64b=0.0;
	double l_float64pn=0.0;
	double l_float64an=0.0;
	double l_float64gam=0.0;
	double l_float64ai=0.0;
	double l_float64cng=0.0;
	double l_float64gain=0.0;
	uint32 l_ui32nc=0;
	uint32 l_ui32jt=0;
	uint32 l_ui32ii=0;
	uint32 l_ui32ir=0;
...
void CComputeTemporalFilterCoefficients::initialize(double float64LowPassBandEdge, double float64HighPassBandEdge, uint32 ui32SamplingRate, uint32 ui32FilterOrder, uint64 ui64FilterType, uint64 ui64FilterMethod)
{
	m_float64LowPassBandEdge 	= float64LowPassBandEdge;
	m_float64HighPassBandEdge 	= float64HighPassBandEdge;
	m_ui32SamplingRate 			= ui32SamplingRate;
	m_ui32FilterOrder			= ui32FilterOrder;
	m_ui64FilterType			= ui64FilterType;
	m_ui64FilterMethod			= ui64FilterMethod;
	//CHANGER
...
void CComputeTemporalFilterCoefficients::process(void)
{
	...
	// {
		if(m_ui64FilterMethod==OVP_TypeId_FilterMethod_Butterworth || m_ui64FilterMethod==OVP_TypeId_FilterMethod_Chebychev)
		{
			...
			//local variables
			double l_float64BandWidth=0.0;
			double l_float64HighFrequencyEdge=0.0;
			double l_float64Ang;
			double l_float64CosAng;
			...
			if(m_ui64FilterType==OVP_TypeId_FilterType_HighPass)
			{
				...
				l_float64HighFrequencyEdge = (double)m_ui32NyquistFrequency;
			}
			else
			{
			...
			// where T = 1/fs
			l_float64Ang = (double)l_float64BandWidth * pi / (double)m_ui32SamplingRate;
			...
			l_float64HighFrequencyEdge = pi * (l_float64HighFrequencyEdge+m_float64LowPassBandEdge) / (double)m_ui32SamplingRate ;
			m_float64CosGam = cos(l_float64HighFrequencyEdge) / l_float64CosAng;
			l_float64HighFrequencyEdge = 2.0 * pi * m_float64HighPassBandEdge / (double)m_ui32SamplingRate;
