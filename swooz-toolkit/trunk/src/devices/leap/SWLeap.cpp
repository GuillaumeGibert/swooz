...
bool SWLeap::init()
{
	...
	/*Leap::Controller::PolicyFlag l_flag = static_cast<Leap::Controller::PolicyFlag>(1); //(1<<0) = 1
	m_controller.setPolicyFlags(l_flag); //Allows background use
	*/
	m_controller.setPolicyFlags(Controller::POLICY_BACKGROUND_FRAMES);
	
	int l_timer = 10000;
	while(!m_controller.isConnected() && l_timer != 0)
	{	
		std::cout<<"Alert!! "<<l_timer/1000<<" seconds left until stopping! Please connect the device"<<std::endl;
		l_timer--;

//        boost::this_thread::sleep( boost::posix_time::seconds(1) ); // TODO
	}
...
bool SWLeap::read()
{
	...
	if(m_controller.isConnected()) //controller is a Controller object
    {
			...
			m_fps=frame.currentFramesPerSecond(); //getting the FPS
			 ...
			 if (!frame.hands().isEmpty())
			 {
				...
				l_temphand = frame.hands()[0];
				
				if(l_temphand.isLeft())
				{
					l_lefthand = frame.hands()[0];

					
					if (frame.hands().count() > 1)
					{

						l_temphand = frame.hands()[1];
				...
				if(l_lefthand.isValid())
				{
					...
					{
						if(!setFingers(frame,l_fingersLeft,m_bisleft))
						{
				...
				if(l_righthand.isValid())
				{
					...
					{
						if(!setFingers(frame,l_fingersRight,m_bisleft))
						{
							std::cout<<"Error Setting fingers Pos/Dir, Default Value will be send "<<std::endl;
						}
					}
